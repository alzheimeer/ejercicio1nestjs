...
describe('UserService', () => {
  ...
  describe('getone usuario', () => {
    it('debería obtener un usuario y su dirección principal correctamente', async () => {
      ...
      userUcMock.getUserAndMainAddress.mockResolvedValue(
        expectedServiceResponse.data,
      );
  ...
  describe('update usuario address', () => {
    it('debería actualizar las direcciones de un usuario correctamente', async () => {
      ...
      await expect(
        userService.updateAddresses(userId, updateAddressDtos),
      ).resolves.toEqual(expectedResponse);
      expect(userUcMock.updateAddresses).toHaveBeenCalledWith(
        userId,
        updateAddressDtos,
      );
    });
  ...
  describe('errores usuarios', () => {
    it('debería manejar errores al obtener todos los usuarios', async () => {
      userUcMock.getAllUsers.mockRejectedValue(
    ...
    it('debería manejar errores al obtener un usuario y su dirección principal', async () => {
      ...
      userUcMock.getUserAndMainAddress.mockRejectedValue(
        new Error(
          'Error al consultar usuario por ID: usuario-inexistente. Por favor, intente de nuevo más tarde.',
        ),
      );
      await expect(userService.getUserAndMainAddress(userId)).rejects.toThrow(
        'Error al consultar usuario por ID: usuario-inexistente. Por favor, intente de nuevo más tarde.',
      );
      expect(userUcMock.getUserAndMainAddress).toHaveBeenCalledWith(userId);
    });
    ...
    it('debería manejar errores al actualizar las direcciones de un usuario', async () => {
      ...
      userUcMock.updateAddresses.mockRejectedValue(
